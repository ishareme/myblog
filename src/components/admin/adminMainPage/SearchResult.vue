            ...
            <el-table-column prop="_id"
                ...
                <template scope="scope">
                    <el-button type="text" size="small" class="viewBtn">
                        <router-link class="viewBtnA" :to="{ path:'/', query: { id: scope.row._id} }">查看</router-link>
                    </el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(){
                let title = this.$route.query.articleTitle
                this.articleTitle=title
                this.$http.post('/api/articleSearchList',{articleTitle:this.articleTitle}).then((res)=>{
                    this.tableData=res.data.data
                    console.log(this.tableData)

                    this.tableData.forEach((item,index)=>{
                        this.filterTagData.push(item.tag)
                        this.filterCategaryData.push(item.categary)
                    })
                    this.filterTagData.forEach((item,index)=>{
                        var  itemArr=item.split(',')
                        this.filterTagDataFinal=this.filterTagDataFinal.concat(itemArr)
                    })
                    this.filterCategaryData.forEach((item,index)=>{
                        var  itemArr=item.split(',')
                        this.filterCategaryDataFinal=this.filterCategaryDataFinal.concat(itemArr)
                    })
                    //利用es6 set集合的元素必须是唯一的特性  进行去重
                    this.filterTagDataFinal = new Set(this.filterTagDataFinal);
                    this.filterCategaryDataFinal = new Set(this.filterCategaryDataFinal);
                    ...
                    this.filterCategaryDataFinal.forEach((item,index)=>{
                        this.filterCategaryDataFinalArr.push({ text: item , value: item })
                    })
